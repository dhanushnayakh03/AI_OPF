...
ENV_NAME = "ENV 2: High Reactive Load & Low Power Factor Stress Test"
ENV_DESC = "Grid operating under high inductive loads (Q_L scaled 130% to 180%, power factor drops)."
...
def run_environment_test(num_scenarios=200):
    ...
    for s in range(num_scenarios):
        # Normal active load (0.8-1.2) with severe reactive load surge (1.3-1.8)
        scale_p = np.random.uniform(0.8, 1.2, size=len(net.load))
        scale_q = np.random.uniform(1.3, 1.8, size=len(net.load))
    ...
    print("\n" + "-"*65)
    print(f"Results Summary across {valid_scenarios} Low Power Factor Scenarios:")
    ...
    ax.set_title("Figure 1: IEEE-14 Topology under High Reactive Loading (Low PF)", fontsize=14, fontweight='bold', color='#111827', pad=15)
    ax.axis('off')
    plt.tight_layout()
    ...
    plt.xlabel('Low Power Factor Scenario Index', fontsize=11)
    plt.ylabel('Transmission Loss (MW)', fontsize=11)
    plt.title('Figure 2: Transmission Loss under High Inductive Load (Baseline vs OPF vs AI)', fontsize=12, fontweight='bold')
    ...
    plt.scatter(losses_opf, losses_ai, color='#3b82f6', alpha=0.7, edgecolors='k', s=45, label='High Q Scenarios')
    ...
    plt.title('Figure 3: AI Generalization vs AC-OPF (Low Power Factor)', fontsize=12, fontweight='bold')
    plt.legend(fontsize=10)
    plt.grid(True, linestyle=':', alpha=0.6)
    plt.tight_layout()
    ...
    plt.title('Figure 4: Bus Voltage Profile under High Reactive Power Demand', fontsize=12, fontweight='bold')
    plt.legend(fontsize=10)
    plt.grid(True, linestyle=':', alpha=0.6)
    plt.tight_layout()
    ...
    plt.title('Figure 5: Branch Loading under Low Power Factor Conditions', fontsize=12, fontweight='bold')
    plt.legend(fontsize=10)
    plt.grid(True, linestyle=':', alpha=0.6)
    plt.tight_layout()
    ...
    plt.close()
    print("[ENV2] All 6 figures saved to 'figures/'.")
